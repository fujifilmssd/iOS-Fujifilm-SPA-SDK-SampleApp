...
Include the Fujifilm SPA SDK in your Xcode project. To add Fujifilm SPA SDK to your Xcode project, you may install via Cocoapods or you may add it manually or via Carthage.
...
```ruby
pod 'Fujifilm-SPA-SDK', '~> 2.4.0'
```
Install the pod by navigating to the project directory in a terminal and running `$ pod install`. If you have already installed the SDK and would like to update to the latest version, run `$ pod update` instead. 

This pod includes a dependency on Braintree to support payment via PayPal. If you do not wish to support payment via PayPal, you may instead choose the 'Lite' pod which does not depend on Braintree and only supports payment via Credit Card. To install the Lite pod, update your Podfile like so:
```ruby
pod 'Fujifilm-SPA-SDK/Lite', '~> 2.4.0'
```
...
#### Carthage (or manual) Installation
...
* Carthage
  * Add the following line to your Cartfile
   ...
   * Run the following command in your terminal
     ...
     ```
* Manual
    * On Github, locate the latest release version (currently 2.4.0) of the SPA SDK in the Releases section, and download the file named `Fujifilm_SPA_SDK_iOS.xcframework.zip ` and unzip it.
    * To support payment via PayPal, you will also need to do the same for Braintree's iOS SDK. Download version 6.32.0 or a compatible version from https://github.com/braintree/braintree_ios 
	
2. Add frameworks to project
* To add frameworks, select your project in the Xcode file explorer. In the main window, the top left corner has a dropdown menu with a list of your projects and targets. Make sure your target is selected (not your project) and switch to the Build Phases tab. Expand the Link Binary With Libraries section and click the + button. Select Add Files and select the following frameworks from your Carthage/build folder (or wherever you manually downloaded to):
  * To support payment via PayPal:
    * `Fujifilm_SPA_SDK_iOS.xcframework`
   ...
   * If you don't want to support PayPal, add the following frameworks instead:
     * `Fujifilm_SPA_SDK_iOS-Lite.xcframework`
     * `FFImagePicker2.xcframework`
 * Notice there are 2 versions of the Fujifilm_SPA_SDK_iOS framework; you should only add one or the other to your project but not both. To support PayPal payments, add `Fujifilm_SPA_SDK_iOS.xcframework`, otherwise add `Fujifilm_SPA_SDK_iOS-Lite.xcframework`
 *  Navigate from the Build Phases tab to the General tab and scroll down to Frameworks, Libraries, and Embedded Content. You should see all of the frameworks you have just linked. Set all of them to Embed & Sign if they aren't already.

* In addition to linking the frameworks from the previous step, link the following iOS frameworks to your project:
- Accelerate
- AddressBook
- AddressBookUI
- AVFoundation
