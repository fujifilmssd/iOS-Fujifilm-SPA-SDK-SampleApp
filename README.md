...
Include the Fujifilm SPA SDK in your Xcode project. To add Fujifilm SPA SDK to your Xcode project, you may add it manually, or you may install via Cocoapods. 
...
1. In your Podfile, include the SPA SDK pod:
   ...
   If it is not already added, make sure your Podfile contains the use_frameworks! instruction. If you are creating a new Podfile, don't forget to include the platform and version your app supports (e.g. platform :ios, '9.0'). 
   
2. Install the pod by running $ pod install using Terminal inside your project directory. Upon successful installation, close your Xcode project if it is open, and open the workspace that was generated by Cocoapods.
...
3. In your view controller class, set your Fujifilm SPA SDK static constants:
   ...
   //A string indicating which environment your app runs in. Must match your app’s environment set on http://fujifilmapi.com. Possible values are “test” or “live”.
   static NSString *const kFujifilmSEnvironment = @"test";
   ```
...
6. When you're ready to launch our SDK, create a Fujifilm_SPA_SDK_iOS object and initialize it using initWithOptions:
   ```objective-c
   Fujifilm_SPA_SDK_iOS *fujifilmOrderController = [[Fujifilm_SPA_SDK_iOS alloc] initWithOptions:kFujifilmSPASDKAPIKey environment:kFujifilmSEnvironment images:fujifilmSPASDKImages userID:userId];
   ```
   ...
   *  **images(id)**: An NSArray of PHAsset, ALAsset, or NSString (public image urls https://). The array can contain a combination of types. Images must be JPG format and smaller than 20MB. A maximum of 50 images can be sent in a given Checkout process. If more than 50 images are sent, only the first 50 will be processed.  
...
7. Set the Fujifilm_SPA_SDK_iOS object’s delegate to the view controller:
   ```objective-c
   fujifilmOrderController.delegate = self;
   ```
   
8. Present the Fujifilm_SPA_SDK_iOS object:
   ```objective-c
   [self presentViewController:fujifilmOrderController animated:YES completion:nil];
   ```
   
9. Implement the delegate. The FujifilmSPASDKDelegate requires your view controller to implement the method fujifilmSPASDKFinishedWithStatus:(int) statusCode andMessage(NSString*) message. 
...
- (IBAction)launchFujifilmSDK:(id)sender {
     ...
     */
      ...
      Fujifilm_SPA_SDK_iOS *fujifilmOrderController = [[Fujifilm_SPA_SDK_iOS alloc] initWithOptions:@"5cb79d2191874aca879e2c9ed7d5747c" environment:@"test" images:fujifilmSPASDKImages userID:@""];
